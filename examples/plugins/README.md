...
host call — an archive has its own 30 s), the same bytes the
watch writes, so a file already imported over USB is skipped by hash — hand a
Garmin download over as it came, no unzipping in the sandbox. Every import refreshes the app's
