...
import { drawAverageLine, rangeSpanning } from "./chartAverage";
import {
  CADENCE,
  ELEVATION,
  HR,
...
import { useUnits, isImperial, distanceUnit, M_PER_MILE, MPH_PER_MPS } from "../../lib/units";

export type { SummaryAverages } from "./chartConfigs";
...
// hasData, resolveAverages) lives in chartConfigs.ts, tested without a canvas.
...
function SingleChart({
  ...
  useEffect(() => {
    ...
    // and lines. Canvas coordinates are device pixels (bbox / valToPos(…,
    // true)), scaled by uPlot's own cached ratio — the one its bbox uses.
    const drawAverage = (u: uPlot) => {
      if (avgValue == null || !isFinite(avgValue)) return;
      drawAverageLine(u.ctx, {
        bbox: u.bbox,
        linePos: u.valToPos(avgValue, "y", true),
        pxRatio: uPlot.pxRatio,
        text: `avg ${fmtVal(avgValue)}`,
        ink: inkColor,
...
export function ChartPanel({
  ...
  // Per-metric value for the "avg" reference line (resolveAverages: summary
  // first, samples as the fallback). Keyed on the summary's primitives:
  // callers pass a fresh object literal per render, and the pace fallback's
  // speedSeries pass is O(N).
  const { hr: sumHr, power_w: sumPower, cadence: sumCadence, speed_mps: sumSpeed } =
    summaryAverages ?? {};
  const averages = useMemo(
    () =>
      resolveAverages(available, seriesByKey, trackpoints, {
        hr: sumHr,
        power_w: sumPower,
        cadence: sumCadence,
        speed_mps: sumSpeed,
